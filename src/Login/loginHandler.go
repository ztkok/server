...
func (s *Server) loginHandler(w rest.ResponseWriter, r *rest.Request) {
	...
	if uid, err = dbservice.GetUID(msg.User); uid == 0 {
		if err != nil && err != redis.ErrNil {
			log.Error("GetUID ERR:", err, "Msg:", msg)
	...
	SessInfo := dbservice.SessionUtil(uid)
	token, terr := SessInfo.GetToken()
	svrID, serr := SessInfo.GetGatewaySvrId()
	if token != "" && terr == nil && svrID != 0 && serr == nil {
		svrInfo, err := serverMgr.GetServerMgr().GetServerById(iserver.ServerTypeGateway, svrID)
		if svrInfo != nil && err == nil {
			addr = svrInfo.OuterAddress
		}
	}
	...
	//获取大厅服务器信息
	svrInfo, err := serverMgr.GetServerMgr().GetServerByType(iserver.ServerTypeGateway)
	if err == serverMgr.ErrServerBusy {
		log.Warn(err, msg)
		rest.Error(w, "服务器忙", http.StatusNotAcceptable)
		return
	}
	if err != nil || svrInfo == nil {
		log.Error(err, msg)
		rest.Error(w, "服务器正在维护", http.StatusInternalServerError)
		return
	}

	addr = svrInfo.OuterAddress
	SessInfo.SetGatewaySvrId(svrInfo.ServerID)
