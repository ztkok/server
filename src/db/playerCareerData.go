...
func PlayerCareerDataUtil(typ string, uid uint64, season int) *playerCareerDataUtil {
...
func (u *playerCareerDataUtil) IsKeyExist() bool {
	c := dbservice.Get()
	defer c.Close()
...
func (u *playerCareerDataUtil) key() string {
	return fmt.Sprintf("%s:%d:%d", u.typ, u.season, u.uid)
}
