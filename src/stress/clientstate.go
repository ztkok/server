...
func (state *RoomPlayerState) Marshal() []byte {
	...
	for i := 0; i < space.EntityStateMask_Max; i++ {
		if i <= space.EntityStateMask_Reserve {
			state.CompareWithZeroAndSetBaseValueDelta(&mask, uint32(i), bs)
		} else {
			state.CompareWithZeroAndSetExtValueDelta(&mask, uint32(i), bs)
		}
	}
...
// CompareWithZeroAndSetExtValueDelta 比较和设置额外的值
func (state *RoomPlayerState) CompareWithZeroAndSetExtValueDelta(mask *int32, maskoffset uint32, bs *common.ByteStream) bool {
	var newfloat float32
	var newByte byte
	var t int
	...
	if t == 1 {
		if math.Abs(float64(newfloat)) <= 0.001 {
			return true
		}
		bs.WriteFloat32(newfloat)
	} else if t == 2 {
